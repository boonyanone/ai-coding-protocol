...
- **แยก Account อิสระตามโปรเจกต์ (Project-Local Vault):** ระบบจะจัดการแฮก/Patch ตัว MCP ให้จัดเก็บข้อมูลการ Login แยกย่อยลงในโฟลเดอร์โปรเจกต์ (`.ai/mcp/auth/auth.json`) แทนการเก็บรวมที่ส่วนกลาง ทำให้คุณสามารถ **ใช้ Google Account แยกกันในแต่ละโปรเจกต์ได้โดยไม่ตีกัน**!

---

## 🛡️ ความปลอดภัยระดับ Enterprise (Enterprise-Grade Security)

การปล่อยให้ AI มารันคำสั่งในเครื่องเราจำเป็นต้องมีระบบรักษาความปลอดภัยขั้นสูงสุด โปรโตคอลนี้ถูกสร้างเป็นป้อมปราการป้องกันการโจมตีจากทั้งภายนอกและความผิดพลาดของตัว AI เอง:
- **ป้องกันการถูกเขียนทับ (Symlink Hijacking & Race Conditions):** ระบบจัดการไฟล์ทั้งหมดใช้การสุ่มชื่อไฟล์ด้วยการเข้ารหัสขั้นสูง (`crypto.randomBytes`) และล็อกการเขียนไฟล์ระดับ OS (`wx` / `COPYFILE_EXCL`) ทำให้ไม่มีไฟล์ไหนสามารถถูกสวมรอยหรือเขียนทับได้เลย
- **ป้องกันเน็ตเวิร์คค้าง (Network DoS Resilience):** ระบบตั้งเวลา Timeout ไว้ที่ 5 วินาที เพื่อป้องกันไม่ให้ AI ทำให้สคริปต์ค้างจากการพยายามดาวน์โหลดไฟล์หรือเชื่อมต่อเซิร์ฟเวอร์ที่ไม่ตอบสนอง
- **สแกนความลับทะลุทะลวง (Deep Git Audits):** ระบบตรวจจับไฟล์ `.env` ที่ดักลึกไปถึงทุกโฟลเดอร์ย่อย แม้คุณจะเผลอ Force Add ไฟล์ความลับเข้าไป Git Hook ก็จะหยุดยั้งการ Commit ให้ทันที

    ...
    try {
      ...
      if (fs.existsSync(srcDir)) {
        ...
        for (const file of files) {
          ...
          // Replace both os.homedir() and home variations
          content = content.replace(/path\.join\(os\.homedir\(\),\s*'\.notebooklm-mcp'/g, "path.join(process.cwd(), '.ai', 'mcp', 'auth'");
          content = content.replace(/path\.join\(home,\s*"\.notebooklm-mcp"/g, "path.join(process.cwd(), '.ai', 'mcp', 'auth'");
